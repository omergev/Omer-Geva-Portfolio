    ...
    <AnimatedSection className="flex flex-col justify-center min-h-[70vh] lg:min-h-[80vh] p-10 md:pe-56" id="education">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex-grow">
          <h2 className="text-2xl font-semibold">{education_bsc.university}</h2>
          <p className="text-lg">{education_bsc.degree}</p>
          <p className="mt-2 text-gray-700">{education_bsc.description}</p>
        </div>
        {/* Dates */}
        <div className="whitespace-nowrap text-right text-gray-500">{education_bsc.period}</div>
      </div>
      {/* Skills or Subjects */}
        <div className="flex flex-wrap gap-2 mt-2">
            {education_bsc.courses.map((course, index) => (
                <span
                key={index}
